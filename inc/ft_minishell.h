...
typedef enum e_builtin_t	t_builtin_t;
...
int			treat_heredocs(t_line *line,
				t_word_list **alst, size_t *here_doc_len);
int			fill_heredocs(t_here_doc *h_doc_tab,
				size_t h_doc_len, t_line *line);

void		ft_builtin(char **argv, size_t count,
				t_exp_list **alst, t_builtin_t type);
...
void		key_is_not_valid(char *program_name,
...
void		fill_and_expand_args_or_cmd(
				t_line *line, t_command *cmd, t_word_list *cursor,
